...
class minimax_Board{
    ...
    public:
        ...
        void print_current_board(int i, int j, int round) { // Print cout the current state of the hole board
            int orb_num;
            char symbol;

            ////// Print cout the current state of the board //////
...
int minimax(minimax_Board board_minimax, int first_depth, int depth, bool maxplayer, char color, char another_color, int a, int b, int method);
bool endgame(minimax_Board board_minimax);
bool inside_range(int row, int col);
int score1(minimax_Board board_minimax, const char &color, const char &another_color);
int score2(minimax_Board board_minimax, const char &color, const char &another_color);
...
void algorithm_B(Board board, Player player, int index[]){
    ...
    cout << "ST4 :" << orb_num << endl;
    if (orb_num > 20)
        position = minimax(board_minimax, 5, 5, true, color, another_color, alpha, beta, 2);
    else
        position = minimax(board_minimax, 4, 4, true, color, another_color, alpha, beta, 1);
    index[0] = position / 10;
    index[1] = position % 10;
    
}
    
int minimax(minimax_Board board_minimax, int first_depth, int depth, bool maxplayer, char color, char another_color, int alpha, int beta, int method) {
    ...
    if (depth == 0 || endgame(board_minimax)) {
        if (method == 1)
            return score1(board_minimax, color, another_color);
        else 
            return score2(board_minimax, color, another_color);
    }
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 6; j++) {
                if (board_minimax.get_cell_color(i, j) != another_color) {
                    ...
                    value_each_time = minimax(board_minimax, first_depth, depth - 1, false, color, another_color, alpha, beta, method);
        ...
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 6; j++) {
                if (board_minimax.get_cell_color(i, j) != color) {
                    ...
                    value_each_time =  minimax(board_minimax, first_depth, depth - 1, true, color, another_color, alpha, beta, method);
    ...
    for (int row = 0; row < 5; row++) {
        for (int col = 0; col < 6; col++) {
            if (board_minimax.get_cell_color(row, col) == color) {
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        if (inside_range(row + i, col + j) && board_minimax.get_cell_color(row, col) == 'w') {
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        if (inside_range(row + i, col + j)) {
                            ...
                            if (difference == 1 && board_minimax.get_cell_color(row + i, col + j) == color)
                                sum += 60 / board_minimax.get_capacity(row + i, col + j);
                            else if (difference == 1 && board_minimax.get_cell_color(row + i, col + j) == another_color)
                                sum -= 60 / board_minimax.get_capacity(row + i, col + j);
                            difference = 8;
                        }
                    }
