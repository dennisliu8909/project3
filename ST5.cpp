...
int score1(min_Board board_min, const char &color, const char &another_color);
int score2(min_Board board_min, const char &color, const char &another_color);
...
void algorithm_B(Board board, Player player, int index[]){
    ...
    /*else if (orb_num <= 60)
        position = min(board_min, 4, 4, true, color, another_color, alpha, beta, 2);*/
    else 
        position = min(board_min, 5, 5, true, color, another_color, alpha, beta, 2);
    index[0] = position / 10;
    index[1] = position % 10;
...
int min(min_Board board_min, int first_depth, int depth, bool maxplayer, char color, char another_color, int alpha, int beta, int method) {
    ...
    if (depth == 0 || endgame01(board_min)) {
        if (method == 1)
            return score1(board_min, color, another_color);
        else 
            return score2(board_min, color, another_color);
    }
